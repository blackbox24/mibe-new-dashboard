...
export const getOtherListingWithPagination = async (resource: string ,page: number = 1, limit: number = 10): Promise<{
  ...
  const response = await api.get(`/other-listings/paginated/?page=${page}&limit=${limit}&filter=${resource}`);
...
export const addOtherListing = async (
  ...
  const response = await api.post(`/other-listings/`,data, {
      headers:{
...
export const updateOtherListing = async (
  ...
  const response = await api.put(`/other-listings/${id}`, {...otherListing},{
      headers:{
        "Content-Type":"application/json"
      }
    })
...
export const deleteOtherListing = async (id: number): Promise<void> => {
  const response = await api.delete(`/other-listings/${id}`);
...
export const publishOtherListing = async(id: number, published: { published: string }): Promise<OtherListing> => {
    ...
    const response = await api.post(`/other-listings/published/${id}`,data,{
      headers:{
        "Content-Type":"application/json"
      }
    });
